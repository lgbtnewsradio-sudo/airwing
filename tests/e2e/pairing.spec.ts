...
test('pairing dialog recovers from a wrong code and then pairs through the UI', async () => {
  ...
  const device = page.locator('.device', { hasText: 'Mock Apple TV' });
  await expect(device).toBeVisible();
  await device.locator('.menu > button').hover();
  await device.locator('.menu-items button', { hasText: 'Pair with code' }).click();
  ...
  await expect(device.locator('.device-meta')).toContainText('paired');
